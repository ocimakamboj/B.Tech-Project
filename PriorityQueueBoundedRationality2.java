...
W - wait time of a customer that he estimated based on the assumption that all the arriving customers will join the queue
...
public class PriorityQueueBoundedRationality2{
	...
	public PriorityQueueBoundedRationality2(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost, double b){
		int capacity = 20000;
		globalT = 0;
	...
	public void EventRun(){
		...
		while(true){
			...
			if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
				//basing the utility on the the assumption that all the arriving customers will join
				double checkStability = 0;
				for(int i=1; i<index; i++){
					checkStability= checkStability + arrivalRates[i]/(noOfServers*serviceRates[i]);
				}
				if(checkStability <=1){
					waitTime = 0;
					for(int i=1; i<=index; i++){
						waitTime = waitTime + nSystem[i]/(noOfServers*serviceRates[i]);
					}
					double denom = 0;
					for(int i=1; i<index; i++){
						denom = denom + arrivalRates[i]/(noOfServers*serviceRates[i]);
					}
					denom = 1 - denom;
					waitTime = waitTime + 1/serviceRates[index];
					waitTime = waitTime/denom;
				}else{
					waitTime = Double.POSITIVE_INFINITY;
				}
	...
	public void PrintStats(){
		System.out.println("for PriorityBoundedRationality2");
		for(int i = 1; i<=noOfClasses ; i++){ 
