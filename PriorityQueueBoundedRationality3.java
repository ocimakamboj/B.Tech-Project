...
W - wait time of a customer that we estimate by running another simulation. (using WaitTimeBoundedRationality.java)
	In this calculation of wait time, arriving customers can balk. 
	The arrival and service processes are deterministic instead of probabilistic, 
	with their parameters being equal to the arrival rate and service rate respectively

Upon arrival, the customer will have expected Utility, U = 0, if he balks
and expected Utility, U = R-p-C*W if he joins

Assuming bounded rationality, the customer joins with the probability - exp(U/b)/(1+exp(U/b))
...
public class PriorityQueueBoundedRationality3{
	...
	public PriorityQueueBoundedRationality3(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost, double b){
		int capacity = 20000;
		globalT = 0;
	...
	public void EventRun(){
		...
		while(true){
			...
			if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
				//WaitTime tells the time spent strictly in waiting
				WaitTimeBoundedRationality var = new WaitTimeBoundedRationality(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService[1], index, beta);
				//System.out.println("Arrival of class " + index);
				//System.out.println("1");
				waitTime = var.calculateWaitTime() + 1/serviceRates[index];
	...
	public void PrintStats(){
		System.out.println("for PriorityBoundedRationality3");
		for(int i = 1; i<=noOfClasses ; i++){ 
		...
		System.out.println();
		System.out.println("%age of people joining : " + ((nDepartures[1]*100)/(nArrivals[1])));
		System.out.println();
		for(int i = 1; i<=noOfClasses ; i++){ 
		...
		System.out.print("Social Welfare is: ");
		System.out.println(SocialWelfare());
		System.out.println();
	}
