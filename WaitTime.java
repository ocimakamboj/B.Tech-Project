public class WaitTime{
	...
	public WaitTime(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost, int[] NInSystem, int[] NInService, int[] InService, int index){
		int capacity = 20000;
		time = 0;
		nSystem = new int[k+1];
		for(int i=0; i <= k; i++){
			...
			if(nSystem[0] < noOfServers){
				...
				if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
					...
					if(index == 1){
						...
						WaitTime var = new WaitTime(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService,index); //
