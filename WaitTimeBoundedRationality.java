...
public class WaitTimeBoundedRationality{
	double time;
	...
	public WaitTimeBoundedRationality(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost, int[] NInSystem, int[] NInService, int[] InService, int index, double b){
		int capacity = 20000;
		time = 0;
		nSystem = new int[k+1];
		for(int i=0; i <= k; i++){
		...
		for(int i=1; i<=k; i++){
			nextArrivalTime[i] = 1/arrivalRates[i];
		}
		nextServiceComp = new double[s+1];
		for(int i=1 ; i<=s ; i++){
			nextServiceComp[i] = 1/serviceRates[inService[i]]; //
		}
		endTime = t;
		noOfServers = s;
		noOfClasses = k;
			...
			if(nSystem[0] < noOfServers){
				...
				if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
					...
					if(index == 1){
						...
						WaitTimeBoundedRationality var = new WaitTimeBoundedRationality(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService,index, beta); //
					...
					if(varR.nextDouble() > prob){
						...
						nextArrivalTime[index] = time + 1/arrivalRates[index];
					}else{
						time = nextArrivalTime[index];
						nextArrivalTime[index] = time + 1/arrivalRates[index];
						...
						for(int i = 1; i <= noOfServers; i++){
							if(inService[i] == 0){
								...
								serviceTime = 1/serviceRates[index];
					...
					if(nSystem[0] < noOfServers){
						...
						serviceTime = 1/serviceRates[newCustomerClass];
