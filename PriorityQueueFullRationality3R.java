...
W - wait time of a customer that we estimate by running another simulation. (using WaitTimeR.java)
...
public class PriorityQueueFullRationality3R{
	...
	public PriorityQueueFullRationality3R(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost){
		int capacity = 20000;
		globalT = 0;
	...
	public void EventRun(){
		...
		while(true){
			...
			if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
				...
				WaitTimeR var = new WaitTimeR(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService[1], index);
	...
	public void PrintStats(){
		System.out.println("for PriorityQueueFullRationality3R");
		for(int i = 1; i<=noOfClasses ; i++){ 
