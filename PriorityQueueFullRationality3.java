...
W - wait time of a customer that we estimate by running another simulation. (using WaitTime.java)
...
public class PriorityQueueFullRationality3{
	...
	public PriorityQueueFullRationality3(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost){
		int capacity = 20000;
		globalT = 0;
	...
	public void EventRun(){
		...
		while(true){
			...
			if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
				...
				WaitTime var = new WaitTime(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService[1], index);
	...
	public void PrintStats(){
		System.out.println("for PriorityQueueFullRationality3");
		for(int i = 1; i<=noOfClasses ; i++){ 
