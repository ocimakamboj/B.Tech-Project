...
W - wait time of a customer that we estimate by running another simulation. (using WaitTimeBoundedRationalityR.java)
	In this calculation of wait time, arriving customers can balk. 
	The arrival and service processes are probabilistic, 
...
public class PriorityQueueBoundedRationality3R{
	...
	public PriorityQueueBoundedRationality3R(double t, int s, int k, double[] ArrivalParameter, double[] ServiceParameter, double[] Reward, double[] price, double[] Cost, double b){
		int capacity = 20000;
		globalT = 0;
	...
	public void EventRun(){
		...
		while(true){
			...
			if(nextEvent[0] == 0 && nextArrivalTime[index] <= endTime){
				//WaitTimeR tells the time spent strictly in waiting
				WaitTimeBoundedRationalityR var = new WaitTimeBoundedRationalityR(endTime - nextArrivalTime[index], noOfServers, noOfClasses, arrivalRates, serviceRates, R, p, C, nSystem, nInService, inService[1], index, beta);
	...
	public void PrintStats(){
		System.out.println("for PriorityBoundedRationality3R");
		for(int i = 1; i<=noOfClasses ; i++){ 
